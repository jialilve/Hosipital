...
public class RX_List_p extends JFrame {
	...
	protected void querysome(ActionEvent e) 
	{
		...
		List<R_X> at =rxdao.querysomeRX_p(this.id,Rxid);
		for(R_X st:at)
		{
			Vector v=new Vector();
			v.add(st.getRxid());
	...
	public void QueryAllRX()
	{
			this.dtm.setRowCount(0);
			RxDao rxdao= new RxDao();
			List<R_X> at =rxdao.queryAllRX_p(this.id);
			for(R_X st:at)
			{
				Vector v=new Vector();
				v.add(st.getRxid());
